...
fn last_category(env: &soroban_sdk::Env) -> Option<NotificationCategory> {
    ...
    // Every event lays its trailing topics out as `[.., category, priority]`, so
    // the category is the second-to-last topic (the last one is the priority).
    let n = topics.len();
    if n < 2 {
        return None;
    }
...
fn test_reduce_usage_nonexistent_group_is_rejected() {
    ...
    client.reduce_usage(&ghost_id, &test_env.users.get(0).unwrap());
}
